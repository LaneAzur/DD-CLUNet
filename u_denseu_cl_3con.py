...
class dens_block(nn.Module):
    def __init__(self,ch_in,ch_out):
        super(dens_block, self).__init__()
...
class Unet(nn.Module):
    def __init__(self, dim = 3,nb_features=None):
        ...
        self.linear_x2 = torch.nn.Linear(32, 32)
        self.linear_y2 = torch.nn.Linear(32, 32)
        self.linear_x3 = torch.nn.Linear(32, 32)
        self.linear_y3 = torch.nn.Linear(32, 32)
    ...
    def forward(self, x,t):
        ...
        f_x1 = self.avgpool(x_enc[4])
        f_x1 = f_x1.squeeze()
        f_x1 = self.linear_x1(f_x1)
        f_x1 = f_x1 / f_x1.norm(dim=-1, keepdim=True)

        f_y1 = self.avgpool(t_enc[4])
        f_y1 = f_y1.squeeze()
        f_y1 = self.linear_y1(f_y1)
        f_y1 = f_y1 / f_y1.norm(dim=-1, keepdim=True)

        f_x2 = self.avgpool(x_enc[3])
        f_x2 = f_x2.squeeze()
        f_x2 = self.linear_x2(f_x2)
        f_x2 = f_x2 / f_x2.norm(dim=-1, keepdim=True)

        f_y2 = self.avgpool(t_enc[3])
        f_y2 = f_y2.squeeze()
        f_y2 = self.linear_y2(f_y2)
        f_y2 = f_y2 / f_y2.norm(dim=-1, keepdim=True)

        f_x3 = self.avgpool(x_enc[2])
        f_x3 = f_x3.squeeze()
        f_x3 = self.linear_x3(f_x3)
        f_x3 = f_x3 / f_x3.norm(dim=-1, keepdim=True)

        f_y3 = self.avgpool(t_enc[2])
        f_y3 = f_y3.squeeze()
        f_y3 = self.linear_y1(f_y3)
        f_y3 = f_y3 / f_y3.norm(dim=-1, keepdim=True)
        ...
        return conv10,f_x1,f_y1,f_x2,f_y2,f_x3,f_y3
...
class VoxelMorph(nn.Module):
    ...
    def forward(self, x: torch.Tensor,y):
        ...
        source = x[:, 0:1, :, :]
        flow,f_x1,f_y1,f_x2,f_y2,f_x3,f_y3 = self.unet_model(x,y)
        ...
        return y_source, pos_flow,f_x1,f_y1,f_x2,f_y2,f_x3,f_y3
